...
4. Note that the ```[keybinding]``` section in ```config.ini``` overrides any core keybindings, and/or user keybindings that you place in the user ```keybinding``` directory. If you just want to make some small tweaks, it's probably better to use the section in ```config.ini```. This functionality is more suited to users who want to do something completely different than a Vi-style workflow.